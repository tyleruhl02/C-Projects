...
int isCyclic2(struct graph* g, int index, int* visited, int* stack) {
    if(stack[index] == 1) {
        return 1;
    }
    if(visited[index] == 1) {
    ...
    while(currentNode->next) {
        currentNode = currentNode->next;
        if(isCyclic2(g, currentNode->num, visited, stack) == 1) {
            return 1;
        }
    }
...
int isCyclic(struct graph* g) {
    int* visited = malloc(g->verticies * sizeof(int));
    int* stack = malloc(g->verticies * sizeof(int));

    for(int i = 0; i < g->verticies; i++) {
        visited[i] = 0;
        stack[i] = 0;
        if(isCyclic2(g, i, visited, stack)) {
            free(visited);
            free(stack);
            return 1;
        }
