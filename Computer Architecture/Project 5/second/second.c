...
void createGreyCodeInputs(bool** input, int dim1, int dim2) {
    ...
    for(int i = 0; i < dim2; i++) {
        for(int j = 0; j < dim1/2; j++) {
            bool temp = input[j][i];
            input[j][i] = input[dim1-j-1][i];
            input[dim1-j-1][i] = temp;
        }
    }
}

bool* toGreyCodeInputs(int num, int dim) {
    bool * output = malloc(dim * sizeof(bool));
    
    bool** allGreyCode = malloc(dim * sizeof(bool*));
    for(int i = 0; i < dim; i++) {
        allGreyCode[i] = malloc((1 << dim) * sizeof(bool));
        for(int j = 0; j < (1 << dim); j++) {
            allGreyCode[i][j] = false;
        }
    }

    createGreyCodeInputs(allGreyCode, dim, (1 << dim));

    //printf("OUTPUT: ");
    for(int i = 0; i < dim; i++) {
        output[i] = allGreyCode[i][num];
        //printf("%d ", output[i]);
    }
    //printf("\n");
    ...
    for(int i = size-1; i >= 0; i--) {
        ...
        while(temp) {
            if(indexes[i] == temp->index) {
                ...
                // if(strcmp(temp->name, "temp1") == 0) {
                //     printf("\n");
                // }
                // printf("GINDEX: %s\t%d\n", temp->name, temp->operation);
...
int greyCodeToInt(bool* greyCode, int size) {
    bool** tempGreyCode = malloc(size * sizeof(bool *));
    for(int i = 0; i < size; i++) {
        tempGreyCode[i] = malloc((1 << size) * sizeof(bool));
    }
    createGreyCodeInputs(tempGreyCode, size, (1 < size));

    for(int i = 0; i < (1 << size); i++) {
        bool equal = true;
        for(int j = size-1; j >= 0; j--) {
            if(tempGreyCode[j][i] != greyCode[size-j-1]) {
                equal = false;
                break;
            }
            //printf("%d ", tempGreyCode[j][i]);
        }
        if(equal) {
            for(int i = 0; i < size; i++) {
                free(tempGreyCode[i]);
...
bool computeOperation(bool* inputs, int numOfInputs, int operation, bool* oppoInputs) {
    ...
    if(operation == 3) { // NOT
        ...
        bool* greyCode = malloc(numOfSelectors * sizeof(bool));
        for(int i = 0; i < numOfSelectors; i++) {
            greyCode[i] = inputs[(1 <<numOfSelectors) + i];
        }

        int k = greyCodeToInt(greyCode, numOfSelectors);
        //printf("k: %d\n", k);
        /*int counter = 1;
        int returnIndex = 0;
        ...
        //     bool* binary = toGreyCodeInputs(i, numOfSelectors);
        //     // printf("i: %d\t", i);
...
int main(int argc, char* argv[]) {
    ...
    // bool* greyCodeLine = toGreyCodeInputs(7, 4);
    // free(greyCodeLine);
    ...
    while(!feof(fp1)) {
        ...
        if(strcmp(word, "OR") == 0) {
            ...
            for(int i = 0; i < (1 << decoderSize); i++) {
                ...
                bool* binary = toGreyCodeInputs(i, decoderSize);
